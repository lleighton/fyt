...
import { Users as UsersIcon } from '@tamagui/lucide-icons'

type ChallengeType = 'amrap' | 'max_effort' | 'for_time' | 'workout'
...
/**
 * Step builder for multi-step challenges
 */
    ...
    <YStack gap="$3">
      <YStack gap="$2">
        ...
        <XStack gap="$2" alignItems="center">
          <Text color="$gray10" fontSize="$3">
            {steps.length < 2 ? (
              <>Add at least <Text fontWeight="700" color="$orange10">2 exercises</Text> to continue</>
            ) : (
              <><Text fontWeight="700" color="$green10">✓</Text> {steps.length} exercises added</>
            )}
          </Text>
        </XStack>
      </YStack>
        ...
        <YStack gap="$3">
          ...
          <YStack gap="$2">
            ...
            <Input
              ...
              onChangeText={(text) => setNewStep({ ...newStep, exercise: text })}
              size="$4"
            />
          </YStack>
            ...
            <XStack gap="$2" flexWrap="wrap">
              {(['reps', 'time', 'distance', 'strength'] as const).map((type) => (
                <Button
                  ...
                  onPress={() => setNewStep({ ...newStep, type })}
                  flex={1}
                  minWidth={80}
                >
                  {type}
              ...
              <XStack gap="$2">
                <YStack flex={1} gap="$2">
                  ...
                  <Input
                    ...
                    onChangeText={(text) =>
                      setNewStep({ ...newStep, target_sets: parseInt(text) || undefined })
                    }
                    size="$4"
                  />
                </YStack>
                <YStack flex={1} gap="$2">
                  ...
                  <Input
                    ...
                    onChangeText={(text) =>
                      setNewStep({ ...newStep, target_reps: parseInt(text) || undefined })
                    }
                    size="$4"
                  />
                </YStack>
              ...
              <YStack gap="$2">
                ...
                <XStack gap="$2">
                  <Input
                    ...
                    onChangeText={(text) =>
                      setNewStep({ ...newStep, target_weight: parseFloat(text) || undefined })
                    }
                    size="$4"
                  />
                  <XStack gap="$1">
                    <Button
                      size="$4"
                      minWidth={60}
                      variant={newStep.weight_unit === 'lbs' ? 'default' : 'outlined'}
                      bg={newStep.weight_unit === 'lbs' ? '$blue10' : undefined}
                      onPress={() => setNewStep({ ...newStep, weight_unit: 'lbs' })}
                    >
                      lbs
                    </Button>
                    <Button
                      size="$4"
                      minWidth={60}
                      variant={newStep.weight_unit === 'kg' ? 'default' : 'outlined'}
                      bg={newStep.weight_unit === 'kg' ? '$blue10' : undefined}
                      onPress={() => setNewStep({ ...newStep, weight_unit: 'kg' })}
                    >
                      kg
                    </Button>
                  </XStack>
...
function ChallengeTypeCard({
  ...
  const bgColor = selected
    ? type === 'amrap' ? '$purple2'
      : type === 'max_effort' ? '$orange2'
      : type === 'for_time' ? '$green2'
      : '$blue2'
    : '$gray2'

  const borderColor =
    type === 'amrap' ? '$purple10'
    : type === 'max_effort' ? '$orange10'
    : type === 'for_time' ? '$green10'
    : '$blue10'
